...
int g_voiceSource = 0; // 0=Self, 1=Team, 2=All
bool g_micStreaming = false;
UINT g_pttKey = 'V'; // Default to 'V'
bool g_waitingForPTT = false;
std::vector<std::wstring> g_blockedIds;

// Globals for tab dialogs
HWND g_hTabDialogs[4] = { nullptr, nullptr, nullptr, nullptr };
const int g_tabDialogIds[4] = { IDD_TAB_MAIN, IDD_TAB_USER, IDD_TAB_INFO, IDD_TAB_SETTINGS };
...
void SpeakFromUI(HWND hTabWnd) {
    ...
    int volume = SendMessage(GetDlgItem(hTabWnd, IDC_VOLUME_SLIDER), TBM_GETPOS, 0, 0);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
    ...
    // Voice source combo
    HWND hSourceCombo = GetDlgItem(hWnd, IDC_SOURCE_COMBO);
    SendMessage(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"Self");
    SendMessage(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"Team");
    SendMessage(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"All");
    SendMessage(hSourceCombo, CB_SETCURSEL, 0, 0); // Default to "Self"

    // Show user info
    SetDlgItemTextW(hWnd, IDC_USER_ID, g_userId);
    SetDlgItemTextW(hWnd, IDC_PREMIUM_STATUS, g_isPremium ? L"Yes" : L"No");

    // Set mic streaming checkbox to unchecked by default
    CheckDlgButton(hWnd, IDC_MIC_STREAM, BST_UNCHECKED);
    g_micStreaming = false;

    // Show default PTT key
    WCHAR keyName[16];
    wsprintf(keyName, L"%c", g_pttKey);
    SetDlgItemTextW(hWnd, IDC_PTT_KEY_DISPLAY, keyName);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG: {
        ...
        for (int i = 0; i < 4; ++i) {
            g_hTabDialogs[i] = CreateDialogParam(
                ...
                hWnd,
                TabDialogProc, // Use the new proc
                0
            );
        ...
        // User Tab
        HWND hTabUser = g_hTabDialogs[1];
        if (hTabUser) {
            HWND hSourceCombo = GetDlgItem(hTabUser, IDC_SOURCE_COMBO);
            SendMessage(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"Self");
            SendMessage(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"Team");
            SendMessage(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"All");
            SendMessage(hSourceCombo, CB_SETCURSEL, 0, 0);

            SetDlgItemTextW(hTabUser, IDC_USER_ID, g_userId);
            SetDlgItemTextW(hTabUser, IDC_PREMIUM_STATUS, g_isPremium ? L"Yes" : L"No");
            CheckDlgButton(hTabUser, IDC_MIC_STREAM, BST_UNCHECKED);
            WCHAR keyName[16];
            wsprintf(keyName, L"%c", g_pttKey);
            SetDlgItemTextW(hTabUser, IDC_PTT_KEY_DISPLAY, keyName);
        }

        // Info Tab
        HWND hTabInfo = g_hTabDialogs[2];
        if (hTabInfo) {
        ...
        // Settings Tab
        HWND hTabSettings = g_hTabDialogs[3];
        if (hTabSettings) {
            HWND hList = GetDlgItem(hTabSettings, IDC_BLOCK_LIST);
    ...
    case WM_COMMAND: {
        ...
        // --- User Tab ---
        case IDC_CHANGE_USER:
            if (DialogBox(hInst, MAKEINTRESOURCE(IDD_LOGIN), hWnd, LoginDlgProc) == IDOK) {
                SetDlgItemTextW(hTabWnd, IDC_USER_ID, g_userId);
                SetDlgItemTextW(hTabWnd, IDC_PREMIUM_STATUS, g_isPremium ? L"Yes" : L"No");

                HWND hVoiceCombo = GetDlgItem(g_hTabDialogs[0], IDC_VOICE_COMBO);
                PopulateVoices(hVoiceCombo);
                SendMessage(hVoiceCombo, CB_SETCURSEL, 0, 0);
            }
            break;
        case IDC_SOURCE_COMBO:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                HWND hSourceCombo = GetDlgItem(hTabWnd, IDC_SOURCE_COMBO);
                g_voiceSource = (int)SendMessage(hSourceCombo, CB_GETCURSEL, 0, 0);
            }
            break;
        case IDC_MIC_STREAM:
            if (HIWORD(wParam) == BN_CLICKED) {
                g_micStreaming = (IsDlgButtonChecked(hTabWnd, IDC_MIC_STREAM) == BST_CHECKED);
                MessageBoxW(hWnd, g_micStreaming ? L"Mic streaming enabled." : L"Mic streaming disabled.", L"Mic Streaming", MB_OK | MB_ICONINFORMATION);
            }
            break;
        case IDC_SET_PTT_KEY:
            g_waitingForPTT = true;
            SetDlgItemTextW(hTabWnd, IDC_PTT_KEY_DISPLAY, L"Press key...");
            SetFocus(hWnd);
            break;
    ...
    case WM_KEYDOWN:
        if (g_waitingForPTT) {
            ...
            WCHAR keyName[16];
            wsprintf(keyName, L"%c", g_pttKey);
            // Always update the key display on the User tab
            SetDlgItemTextW(g_hTabDialogs[1], IDC_PTT_KEY_DISPLAY, keyName);
